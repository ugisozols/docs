...
3. Re-provision with the `wordpress` tag only:

<CodeSwitcher :languages="{cli:'Trellis CLI',manual:'Manual'}">
<template v-slot:cli>

```bash
$ trellis provision <environment> --tags=wordpress
```
...
5. Re-provision again with the `letsencrypt` tag:

<CodeSwitcher :languages="{cli:'Trellis CLI',manual:'Manual'}">
<template v-slot:cli>

```bash
$ trellis provision <environment> --tags=letsencrypt
```
